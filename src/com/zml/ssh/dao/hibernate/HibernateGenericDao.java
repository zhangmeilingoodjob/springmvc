...
import com.zml.ssh.dao.hibernate.support.BeanUtils;
...
public class HibernateGenericDao extends HibernateDaoSupport {
	...
	protected static final Log logger = LogFactory
			.getLog(HibernateGenericDao.class);
	 ...
	 * @return
	 */
	public <T> long getAllCount(Class<T> entityClass) {
		String type = entityClass.getName();
		String clazzName = type.substring(type.lastIndexOf(".") + 1);
		// Count查询
		String countQueryString = " select count (*) from " + clazzName;
		return (Long) getHibernateTemplate().find(countQueryString).get(0);
	}

	/**
	 * 分页查询，返回第pageNo页的所有记录
	 * 
	 * @param entityClass
	 * @param pageNo
	 * @param pageSize
	...
	public <T> Page pagedQuery(Class<T> entityClass, int pageNo, int pageSize) {
		...
		Query query = createQuery("from " + clazzName);
